# -*- coding: utf-8 -*-
"""
Created on Thu Sep 14 15:39:18 2023
...
def sample_path(a, b, t0, t1, Q):
    ...
    if c_prob > n_thresh:
        ...
        n_jumps += 1
        c_prob += np.exp(-m*T) * np.power(m*T, n_jumps) / \
            np.math.factorial(n_jumps) * R[a-1, b-1] / p_ab
        ...
        # else, there are at least two jumps
            ...
            while c_prob < n_thresh:
                ...
                c_prob += np.exp(-m*T) * np.power(m*T, n_jumps) / \
                    np.math.factorial(n_jumps) * R_pow[a-1, b-1, n_jumps] / p_ab
